...
Then ("I set up the name of the scenario as {string}", async function (name) {
    if (fs.existsSync(`./vrt/v4/screenshots/${name}`)){
      fs.rmSync(`./vrt/v4/screenshots/${name}`, { recursive: true, force: true }, err => {
        if (err)
          throw err;
      })
    }
    fs.mkdirSync(`./vrt/v4/screenshots/${name}`);        
    stepCount+=1
    scenarioName = name
})
AfterStep(async function (_world) {
  ...
  return await this.driver.saveScreenshot(
      `./vrt/v4/screenshots/${scenarioName}/${scenarioName}-${this.userId}-${stepCount}.png`
  );
});
...
Then('I filter by scheduled pages', async function() {
  stepCount+=1
  await this.driver.$('body > div.gh-app > div > main > section > div > header > section > div > div.gh-contentfilter-menu.gh-contentfilter-type').click()
  await this.driver.$('body > div > div > ul > li:nth-child(4)').click()
})
